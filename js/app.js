...
RESHOP.isotopeFilter = function() {
    // Definir variáveis locais para os elementos (assumindo que são globais ou definidos em outro lugar)
    var $filterGridWrapper = $('.filter__grid-wrapper');  // Wrapper do grid de produtos
    var $collectionOfFilterBtn = $('.filter__btn');       // Botões de filtro

    // Check if filter grid wrapper is on the page
    if ($filterGridWrapper.length) {
        // Inicializar Isotope no grid de produtos
        $filterGridWrapper.isotope({
            itemSelector: '.filter__item',  // Seletor para itens individuais (produtos)
            filter: '*'  // Mostrar todos por padrão
        });
    }

    // Check if filter buttons are on page then attach click event
    if ($collectionOfFilterBtn.length) {
        // Attach click event to these filter buttons
        $collectionOfFilterBtn.on('click', function() {
            // Get Value of the attribute data-filter (ex.: '*', '.masculino', '.feminino', etc.)
            var selectorValue = $(this).attr('data-filter');
            
            // Verificar se o seletor é válido (evitar erros)
            if (selectorValue) {
                // Now initialize isotope plugin with the selected filter
                $filterGridWrapper.isotope({
                    filter: selectorValue
                });
                
                // Remover classe 'js-checked' de outros botões e adicionar ao clicado
                $(this).closest('.filter-category-container').find('.js-checked').removeClass('js-checked');
                $(this).addClass('js-checked');
            } else {
                console.warn('Filtro inválido: atributo data-filter ausente ou vazio.');
            }
        });
    } else {
        console.warn('Botões de filtro não encontrados na página.');
    }
};


    // Plugin de contagem regressiva Bind
    RESHOP.timerCountDown = function() {
        // Verifique se há uma contagem regressiva na página.
    ...
    RESHOP.modalProductDetailInit = function() {
        if ($modalProductDetailElement.length && $modalProductDetailElementThumbnail.length) {
            ...
            // Conecte-se ao evento "shown" do Bootstrap e acione manualmente o evento "resize"
// para que o Slick recalcule as larguras
